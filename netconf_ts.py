#!/usr/bin/env python3
# netconf_iosxe_fancy_ascii_with_student_xml.py
#
# Narrated NETCONF workflow for Cisco IOS-XE (ASCII-only, color optional).
# Now with **student-paste XML payloads**: learners can paste XML exported
# from YANG Suite to drive edit-config without changing the rest of the script.
#
# What changed vs. the original:
# - Added two paste-here blocks: STUDENT_APPLY_XML and STUDENT_REVERT_XML
# - If either block contains real XML (starts with '<' and ends with '>'),
#   the script will use it as the outgoing <config> payload.
# - If left empty, the script falls back to the auto-generated payloads so
#   behavior stays identical to the original.
# - You can optionally force using the student XML with --use-student-xml
#   (script will error if the block is empty).
#
# ---------------------------------------------------------------
# HOW STUDENTS USE THIS
# 1) In YANG Suite, build the desired config for IOS-XE native model
#    (hostname + Loopback add/remove) and export the **NETCONF XML**.
# 2) Paste ONLY the raw XML (no backticks) into the appropriate block below:
#    - STUDENT_APPLY_XML for the "apply" action
#    - STUDENT_REVERT_XML for the "revert" action
#    Make sure the outer element is <config xmlns="urn:ietf:params:xml:ns:netconf:base:1.0"> ... </config>
# 3) Run this script exactly like the original. If a student block has XML,
#    it will be used; otherwise the script uses its built-in payload.
# 4) To *require* student XML, add --use-student-xml on the CLI.
# ---------------------------------------------------------------
...
# ------------------- STUDENT XML PAYLOADS (PASTE HERE) -------------------
# Paste ONLY the XML payload exported from YANG Suite between the triple quotes.
# Do not add backticks or Python quotes—just raw XML.
# The first line should look like:
#   <config xmlns="urn:ietf:params:xml:ns:netconf:base:1.0"> ... </config>

STUDENT_APPLY_XML = r"""
<!-- Paste APPLY XML from YANG Suite here (sets hostname, creates Loopback) -->
"""

STUDENT_REVERT_XML = r"""
<!-- Paste REVERT XML from YANG Suite here (restores hostname, removes Loopback) -->
"""
# ----------------- END STUDENT XML PAYLOADS (PASTE HERE) -----------------
...
# ------------------- Student XML plumbing -------------------

def _is_real_xml(s: str) -> bool:
    s = (s or "").strip()
    return len(s) > 0 and s.startswith("<") and s.endswith(">")
...
class StudentXML:
    ...
    def get(self, kind: str) -> str | None:
        """Return student XML for 'apply' or 'revert' if present/valid; else None.
        If require=True and missing/invalid, raise ValueError.
        """
        raw = STUDENT_APPLY_XML if kind == "apply" else STUDENT_REVERT_XML
        xml = raw.strip()
        if _is_real_xml(xml):
            return xml
        if self.require:
            raise ValueError(
                f"--use-student-xml was set, but STUDENT_{kind.upper()}_XML is empty or invalid.\n"
                "Paste the exported NETCONF XML (outer <config>...</config>) into the block at the top."
            )
        return None

# ------------------- Apply/Revert ops -------------------

def do_apply(m, p: Printer, target_ds, new_hostname, loop_id, has_candidate, sx: StudentXML):
    # Prefer student XML if provided; otherwise use built-in template
    student = sx.get("apply")
    if student is not None:
        edit_payload = student
        p.info("Using student-provided APPLY XML payload from YANG Suite.")
    else:
        edit_payload = f"""
...
def do_revert(m, p: Printer, target_ds, revert_hostname, loop_id, has_candidate, sx: StudentXML):
    # Prefer student XML if provided; otherwise use built-in template
    student = sx.get("revert")
    if student is not None:
        edit_payload = student
        p.info("Using student-provided REVERT XML payload from YANG Suite.")
    else:
        edit_payload = f"""
...
def main():
    ...
    parser.add_argument("--use-student-xml", action="store_true",
                        help="Require the paste-here blocks to contain XML; error if missing.")
    ...
    # Prepare student XML helper
    sx = StudentXML(require=args.use_student_xml)
    ...
    if args.action == "apply":
        ...
        try:
            do_apply(m, p, target_ds, args.hostname, args.loopback, has_candidate, sx)
        except (RPCError, XMLError, ValueError) as e:
    ...
    if args.action == "revert":
        ...
        try:
            do_revert(m, p, target_ds, revert_hostname, args.loopback, has_candidate, sx)
        except (RPCError, XMLError, ValueError) as e:
